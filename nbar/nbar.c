/*~!nbar.c*/
/* Name:  nbar.c Part No.: _______-____r
 *
...
#ident	"$Id: nbar.c,v 1.2 2019/02/21 03:23:46 jbev Exp $"

/*
 * $Log: nbar.c,v $
 * Revision 1.2  2019/02/21 03:23:46  jbev
 * Ignore <stdlib.h> for mpx
 *
 * Revision 1.1  2019/01/10 23:04:20  jbev
 * Initial revision
 *
...
/*
 * nbar command:
 *	nbar -drtx[cnuv] A[lib/dir] file1 file2 ....
 *
...
char *argv[];
{
    ...
    if (!comflg) {
    	fprintf(stderr, "nbar: d, r, t, or x must be specified\n");
    	usage();
    }
    if (comflg > 1) {
    	fprintf(stderr, "nbar: only one of d, r, t, or x can be specified\n");
    	usage();
    }
    ...
    if((lp = cvtfname(tfn)) == (char *)0) {
    	fprintf(stderr, "nbar: can't resolve library name %s\n", argv[optind]);
    	usage();
    }
    ...
    if((dp = cvtfname(tfn)) == (char *)0) {
    	fprintf(stderr, "nbar: can't resolve directory name %s\n", argv[optind]);
    	usage();
    }
    ...
    if (stat(lp, &s1) < 0) {
    	if (xflg || tflg || dflg) {
    	    fprintf(stderr, "nbar: library not present %s\n", lp);
    	    exit(2);
    	}
    	if (!cflg)
    	    fprintf(stderr, "nbar: library not present, creating %s\n", lp);
    	if ((lfd = creat(lp, 0666)) < 0) {
    	    fprintf(stderr, "nbar: can't create library %s\n", lp);
    	    exit(2);
    	}
    ...
    if ((s1.st_mode & S_IFMT) != S_IFREG) {
    	fprintf(stderr, "nbar: library not regular file %s\n", lp);
    	usage();
    }

    /* it's there, see if we can open unblocked */
    if ((lfd = open(lp, O_UNBLK | O_RDWR)) < 0) {
    	fprintf(stderr, "nbar: can't open library %s\n", lp);
    	exit(2);
    }
    ...
    if (stat(dp, &s2) < 0) {
    	if (xflg || tflg || dflg) {
    	    fprintf(stderr, "nbar: directory not present %s\n", dp);
    	    exit(2);
    	}
    	if (!cflg)
    	    fprintf(stderr, "nbar: directory not present, creating %s\n", dp);
    	if ((dfd = creat(dp, 0666)) < 0) {
    	    fprintf(stderr, "nbar: can't create directory %s\n", dp);
    	    exit(2);
    	}
    ...
    if ((s2.st_mode & S_IFMT) != S_IFREG) {
    	fprintf(stderr, "nbar: directory not regular file %s\n", dp);
    	usage();
    }

    /* it's there, see if we can open */
    if ((dfd = open(dp, O_UNBLK | O_RDWR)) < 0) {
    	fprintf(stderr, "nbar: can't open directory %s\n", dp);
    	exit(2);
    }
...
#endif
    	if (xrc <= 0) {		/* see if EOF */
    	    fprintf(stderr, "nbar: can't write to library %s\n", lp);
    	    exit(2);
    	}
    	memset(obuf, 0, BLKSIZE);	/* clear output record */
...
#endif
    	if (xrc <= 0) {		/* see if EOF */
    	    fprintf(stderr, "nbar: can't write to directory %s\n", dp);
    	    exit(2);
    	}
    ...
    if (ruflg && !rflg) {
    	fprintf(stderr, "nbar: u option specified without r option\n");
    	usage();
    }

    /* see if n option present without x option */
    if (xnflg && !xflg) {
    	fprintf(stderr, "nbar: n option specified without x option\n");
    	usage();
    }
    ...
    if (tflg) {
    	...
    	while ((ip = readl(lfd,120)) > (char *)0) {
    		...
    		if (getname (ip, name) < 0) {
    		    /* bad object, abort */
    		    fprintf(stderr, "nbar: bad object in file %s\n", lp);
    		    exit(2);		/* done */
    		}
    		printf ("%s\n", name);
    	    }
    ...
    if (xflg) {
    	...
    	while ((ip = readl(lfd,120)) > (char *)0) {
    		...
    		if (!bypass) {
    			...
    			fprintf(stderr, "nbar: can't create object file %s\n", name);
    			bypass = 1;
    		    } else
    			if(vflg)
    			    printf ("extracting %s\n", name);
...
#endif
    		if(nr <= 0) {
    		    fprintf(stderr, "nbar: can't write object file %s\n", name);
    		    close(cffd);
    		    bypass = 1;
    		}
    	    }
    ...
    if (dflg || (rflg && !cflg)) {
    	...
    	if ((tlfd = open(ip, O_UNBLK | O_TRUNC | O_RDWR | O_CREAT, 0666)) < 0) {
    	    fprintf(stderr, "nbar: can't open temp library %s\n", "templib");
    	}
    	/* get its type/size for temp file creation */
    	type = s2.st_nlink;
    	...
    	if ((tdfd = open(ip, O_UNBLK | O_TRUNC | O_RDWR | O_CREAT, 0666)) < 0) {
    	    fprintf(stderr, "nbar: can't open temp directory %s\n", "tempdir");
    	}
#ifdef mpx
    ...
    if (dflg) {
    	...
    	if ((argc-optind) <= 1) {
    	    fprintf(stderr, "nbar: no objects specified for delete\n");
    	    usage();
    	}
    	bypass = 0;
    	tlibcnt = 0;
...
doneit:;
    		...
    		if(nr <= 0) {
    		    fprintf(stderr, "nbar: can't write temp lib file\n");
    		    close(tlfd);
    		    bypass = 1;
    		}
    		oneflg = 0;
    		while ((ip = getdef(oneflg, ip, name, form)) != (char *)0) {
    		    ...
    		    if(nr <= 0) {
    			fprintf(stderr, "nbar: can't write temp dir file\n");
    			close(tdfd);
    			bypass = 1;
    			continue;
    		    }
    	...
    	if (!bypass) {
    	    nr = writc(tlfd);
    	    if(nr <= 0) {
    		fprintf(stderr, "nbar: can't close temp lib file\n");
    		close(tlfd);
    	    }
    	}
    	...
    	if (!bypass) {
    	    nr = writdc(tdfd);
    	    if(nr <= 0) {
    		fprintf(stderr, "nbar: can't close temp dir file\n");
    		close(tdfd);
    	    }
    	}
    ...
    if (rflg) {
    	...
    	/* more than one arg must be specified */
    	if ((argc-optind) <= 1) {
    	    fprintf(stderr, "nbar: no objects specified for replace\n");
    	    usage();
    	}
    	bypass = 0;
    	libcnt = 0;
    	...
    	while ((ip = readl(lfd,120)) > (char *)0) {
    		...
    		for (rc = optind+1; rc < argc; rc++) {
    			...
    			if (!stricmp(name, basename(argv[rc]))) {
#ifdef DBUG
fprintf(stderr, "nbar/replace: testing %s against arg %d file %s\n",
...
addfiles:
    	for (rc = optind+1; rc < argc; rc++) {
    		...
    		fprintf(stderr, "nbar: can't open object file %s\n", argv[rc]);
    		continue;
    	    } else {
readobj:
    		ip = fbuf;
...
#ifdef DBUG
fprintf(stderr, "nbar/adding: testing %s against arg %d file %s\n",
 name, rc, basename(argv[rc]));
#endif
    		    cblk = tlibcnt;	/* current blk in temp dir */
    		    crec = rob;		/* current rec in tmp dir */
...
void usage()
{
    fprintf(stderr, "Usage: nbar [-drtx][cnuv] Archive f1 f2 ... fn\n");
    exit(2);
}
