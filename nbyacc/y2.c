...
void lhsfill( s )	/* new rule, dump old (if exists), restart strings */
	char *s;
{
	rhsfill( (char *) 0 );
...
void lrprnt()	/* print out the left and right hand sides */
{
	char *rhs;
...
void beg_debug()	/* dump initial sequence for fdebug file */
{
	fprintf( fdebug,
...
void end_toks()	/* finish yytoks array, get ready for yyred's strings */
{
...
void end_debug()	/* finish yyred array, close file */
{
