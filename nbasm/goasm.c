...
unsigned char	spmac;		/* HOLDS FLAGS FOR SPECIAL CONDITIONS */
				/* IN MACRO PROCESSING. */
short	hhodp;			/* MASTER VARIABLE FOR OUTPUT INFO */
/* short	hhodpa;			/* ALTERNATE FOR HHODP */
...
void	aerr()
{
	...
	if (yeanay()) {			/* are we assembling */
	  ...
	  if (mep) {			/* see if macro found */
	    ...
	    macd.value = (int32)dmstp;	/* the pointer to macss entry */
	...
	if ((!(PASS & 1)) && symtab) {	/* are we in pass 2 */
	  ...
	  for (p = symtab; p->flink; p = p->flink) { /* do all symbols */
	    ...
	    if(p->desc.type == TYPX) { 	/* see if external symbol */
	      ...
	      sbp->dsdesc.value = (unsigned int32)0;
	      /* clear csect sb address */
	      sbp->csdesc.value = (unsigned int32)0;
	    ...
	    if (*p->sym != '!') {	/* do not do internal symbols */
	      /* give the symbol to xref'er */
	      reptr(p->sym, &p->desc, p->desc.value & 0x7fffff);
	    }
#endif
	  }
	}
...
dodsect:
	...
	bouc(asctbvl.value);		/* bound new PC */
...
void	cdl()
{
	...
	gen16((expr[0].value > 1) & 0x7, (expr[0].value >> 4) & 0x7 , paug);
	return;
}
...
{ 
	...
	/* gen 16 bit opcode */
	gen16(expr[0].value, expr[1].value & 0x7, preg);
	return;
}
...
{
	/* source register is zero */
	gen16(0, expr[0].value, curops->aug);		/* gen 16 bit opcode */
	return;
}
...
void	calm()
{
	...
	*/
	...
	gen16(((expr[0].value & 0x700) >> 4),
	  ((expr[0].value & 0x380) >> 7), expr[0].value & 0xf);
	return;
}
...
void	cf2d()
{
	...
	/* gen 16 bit opcode */
	    ...
	    gen16a(0x09, expr[0].value, 0x00, 0x0e);	/* TRFW SR */
	    gen16a(0x29, expr[0].value, 0x02, 0x0f);	/* TDPR SR */
	} else {
	    gen16a(0x0d, expr[0].value+1, expr[0].value+1, 0x00); /* ZR SR+1 */
	}
}
...
void	cd2f()
{
	...
	/* gen 16 bit opcode */
    	if (option & OPT55) { 		/* option 55 set (IEEE) */
	    gen16a(0x09, expr[0].value, 0x00, 0x0f);	/* TRFD SR */
	    gen16a(0x29, expr[0].value, 0x02, 0x0d);	/* TSPR DR */
	}
}
...
void	rsc1()
{
    	...
    	/* use reg as sr value */
	/* gen 16 bit opcode */
	gen16(expr[0].value, curops->varcode, curops->aug);
}
...
{
	/* gen 16 bit opcode */
	gen16(expr[1].value, expr[0].value, curops->aug);
	return;
}
...
void	fpctx()
{
	...
	/* gen 32 bit opcode */
	gen32(expr[1].value, xr, fakeaddr);
}
...
void	shf()
{
	...
	if (DEFMBODY & macstate) {		/* are we in body of def */
	  ...
	  gen16(0, expr[0].value, 0);		/* gen 16 bit opcode */
	} else
	...
	/* gen 16 bit opcode */
	gen16(preg, expr[0].value, expr[1].value & 0x0f);
}
...
void	zrx()
{
	...
	/* gen 16 bit opcode */
	gen16(expr[0].value, expr[0].value, curops->aug);
	return;
}
...
{
	/* gen 16 bit opcode */
	gen16(expr[0].value, expr[1].value, curops->aug);
	return;
}
...
*/
	...
	newadr = ageq (expr[1].value, expr[1].type);
...
void	io()
{
	...
	gr = (expr[0].value & 0x7) >> 4;	/* gr is last 3 bits of dev */
...
*/
	...
	if (!(bits3 & EXPLFC)) {	/* check for csect or non zero dsect */
	  ...
	  /* give the symbol to xref'er */
	  reptr(dsname, &sectpc[PCDS], sectpc[PCDS].value);
	  /* code section name and size */
	  /* give the symbol to xref'er */
	  reptr(csname, &sectpc[PCCS], sectpc[PCCS].value);
	}
	...
	while (stp) {			/* scan all of symbol table */
	  if (stp->desc.type == TYPP) {	/* is symbol type local */
		...
		*hbbs = 0;		/* reset binary output stack count */
		...
		bfa (stp->desc.value << 3); /* sym addr left shifted 3 bits */
		...
		bfw (stp->desc.value << 3); /* sym addr left shifted 3 bits */
	        memCpy (usname, stp->sym, 8);	/* move 8 bytes of sym name */
	        bfn0a ();		/* output 8 bytes from usname */
	        pb (hbbs, EXPFUNC, 0);	/* put common block def on bo */
#endif
	  ...
	  *hbbs = 0;			/* reset binary output stream */
	  if (hweval.flags & ABSREL) {	/* is address relative */
	    ...
	    if (bits3 & EXPLFC) {	/* are we using exp funct codes */
	      ...
	      bfa0 (hweval.value);	/* put transfer addr on binary stack */
	      pb (hbbs, EXPFUNC, 0);	/* publish terminal transfer address */
	    } else {			/* use std loader function codes */ 
	      bfa (hweval.value);	/* put transfer addr on binary stack */
