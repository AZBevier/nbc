...
#endif
char	inscnt;		/* input scanner count */
char	ppt;		/* binary record length (120) */
...
int	inpt()
{
	...
	uchrfpp	place;			/* which pointer to use */
...
pouu:
	...
	if (!(inptstat & (EXPAND | RPTGEN))) {
	  sprintf(xx, "%0.5d", lict);	/* print line number */
...
*/
	...
	if (buff[2] == 'C') {			/* is type common */
	  tmp = ((int32)(lcd->value & 0xff0000L) >> 16);  /* get common blk # */
	  if (hwcmsize[tmp] & 0x80000000L)	/* is it ssect */
	...
	sprintf(xx, "%0.5X", lcd->value & 0xfffffL);	/* store 5 hex chars */
