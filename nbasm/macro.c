...
#else
	chrp = (uchrfp)malloc((uint32)256);	/* get some memory */
#endif
...
printf("about to move %x bytes\n", rcnt);
	  ...
	  if (*libbuf == 0xcf) {	/* was this final record */
	    ...
	    macd.value = (int32)chrp;	/* set prototype address */
...
#endif
	 lnegate (label, nlabel);	/* make negative version for macros/forms */
	 nlabel[8] = '\0';
	 dmstp = ss (&macx, nlabel);		/* enter macro name */
...
prtsymt();				/* dump the symbol table */
	    ...
	    head->desc.value = (int32)work;	/* set the new addr */
	    }
#else
	head->desc.value = 
	  (int32)realloc((uchrfp)head->desc.value, (uint32)maccnt);
#endif
#ifdef DEBUG
{
char *p = (char *)head->desc.value;
...
#ifdef MACD
	printf("formp: pass 1 processing\n");
#endif
	  /* pass 1 processing */
	  macdptr = macparam;		/* reset dummy parameter pointer */
	  ...
	  lnegate (label, nlabel);	/* make negative version for macros/forms */
	  nlabel[8] = '\0';
	  dmstp = ss (&formx, nlabel);		/* enter macro name */
/*	  dmstp = ss (&formx, csnegate(label)); */ /* enter macro name */
	...
	dmstp->desc.value = (int32)head; /* macro def entry addr */
#ifdef MACD
	printf("formp: maccnt = %x high = %x\n", maccnt, high);
#endif
	}
formp3:					/* pass 2 merge point */
	/* when we get here in pass2, dmstp is reall address of head */
#ifdef MACD
	printf("formp: at formp3 maccnt = %x macsp %x\n", maccnt, (int)macsp);
#endif
	/* get the bit length */
	val(0);				/* nothing unstrung yet */
#ifdef MACD
	printf("formp: formp3 maccnt = %x value %x\n", maccnt, inac.value);
#endif
	...
	inptstat &= ~FORMSCAN;		/* clear in form body flag */
#ifdef MACD
	printf("formp: scan complete dmstp %x macsp %x maccnt = %x\n", (int)dmstp, macsp, maccnt);
#endif
	spmac &= ~DEFFRM2;		/* clear pass2 defm-form flag */
	/* now update macro byte count in mac symbol entry */
	head->desc.tmp = maccnt;	/* bytes read in for macro */
...
#else
	head->desc.value =
	  (int32)realloc((uchrfp)head->desc.value, maccnt);
#endif
#endif
#ifdef MACD
	printf("formp: returning\n");
	prtval("formp dmstp", &dmstp->desc);
	printf("formp: hwcmacp %x data maccnt %x\n", (int)head, maccnt);
	fflush(stdout);
	hwcmacp = macsp - maccnt;
	printf("formp hwcmacp %x", (int)hwcmacp);
	fflush(stdout);
	for (bitv=0; bitv<maccnt; bitv++) {
		printf("%x ", hwcmacp[bitv]);
	}
	printf("\n");
	fflush(stdout);
#endif
...
#ifdef MACD
	int32 bitv;
	printf("Entering formexp()\n");
#endif
	inptstat |= FORMGET;		/* indicate stripping down a form */
	/* get where it is stored */
	hwcmac = (uchrfp)dmstp->desc.value;		/* head */
/*	head = (struct symbol FAR *)dmstp->desc.value;*/		/* head */
	hwcmacp = hwcmac;			/* save pointer for form prototype */
#ifdef MACD
	head = (struct symbol FAR *)dmstp->desc.value;		/* head */
	printf("formexp: data %x%x%x\n", hwcmacp[0], hwcmacp[1], hwcmacp[2]);
	printf("formexp: dmstp %x charp %x\n", (int)dmstp, (int)hwcmacp);
	prtval("formexp dmstp", &dmstp->desc);
	printf("formexp: dmstp data maccnt %x\n", maccnt);
	for (bitv=0; bitv<maccnt; bitv++) {
		printf("%x ", hwcmacp[bitv]);
	}
	printf("\n");
...
#endif
#ifdef MACD
 printf("formexp: calling tall()\n");
#endif
	  tall();			/* allocate current form label */
#ifdef MACD
 printf("formexp: returning from tall()\n");
#endif
	}
#ifdef MACD
 printf("formexp: calling vfd()\n");
#endif
	vfd();				/* have vfd do the processing */
#ifdef MACD
	printf("Returning from formexp()\n");
#endif
}
...
#else
	  head->desc.value =
	    (int32)realloc((uchrfp)head->desc.value, (uint32)(high + 256));
#endif
...
#endif
#ifdef MACS
printf("mbyt2: head = %x, maccnt = %x, high = %x\n", head, maccnt,high);
prtval("mbyt2head", &head->desc);
#endif
	}
	*macsp++ = byte;		/* stor the byte */
#ifdef MACS
printf("mbyt return: head %x maccnt=%x byte = %x high = %x\n",(int)head, maccnt, byte, high);
prtval("mbyt return head", &head->desc);
#endif
	return;				/* return */
}
...
#ifdef DOS
	dumrept->desc.value = (int32)_fmalloc(256);
#else
	dumrept->desc.value = (int32)malloc((uint32)256);
#endif
