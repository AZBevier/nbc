...
#ident	"Make4MPX $Id: mkdir.c,v 1.2 2021/07/05 16:37:08 jbev Exp $"

/*
 * $Log: mkdir.c,v $
 * Revision 1.2  2021/07/05 16:37:08  jbev
 * Fix warnings.
 *
...
char **argv;
{
    ...
    while (--argc) {
    	...
    	if((ip = cvtdname(*argv)) == (char *)0) {
    	    fprintf(stderr, "mkdir: can't resolve path %s\n", *argv);
    	    exit(2);
    	}
    	if (mkdir(ip, 0777) < 0) {
    	    fprintf(stderr, "mkdir: can't make directory %s", *argv);
    	    errors++;
    	}
    	free(ip);
    }
