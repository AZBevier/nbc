...
/*
 * $Log: maketime.c,v $
 * Revision 1.2  2021/07/09 20:41:46  jbev
 * Fix warning errors.
 *
 * Revision 1.1  1995/03/05 01:46:14  root
 * Initial revision
 *
 * Revision 1.3  1995/03/05  01:34:22  jbev
 * Remove & from array name.
 *
 * Revision 1.2  1995/03/05  01:33:09  jbev
 * Fix utime call for mpx.
 *
...
libId(maketId, "$Id: maketime.c,v 1.2 2021/07/09 20:41:46 jbev Exp $")
...
{
#ifdef mpx
	static int times[2]; /* static so unused fields are zero */
#else
	...
	if (!date)
		return 0;
#ifdef mpx
	times[0] = times[1] = str2time(date2str(date, datebuf));
