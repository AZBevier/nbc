 ...
 * Fix warning errors.
 *
 * Revision 1.5  1995/10/17 23:11:20  jbev
 * Change file access rights from 444 to 664 to allow other than
 * owner to lock a file on checkout.  For MPX only.
 *
 * Revision 1.4  1995/06/02  20:53:38  jbev
 * CHange mask to 666 from 777
 *
 * Revision 1.3  1995/04/19  04:06:11  jbev
 * Fix unlink.
 *
 * Revision 1.2  1995/04/19  02:50:40  jbev
...
libId(editId, "$Id: rcsedit.c,v 1.6 2021/07/09 20:41:46 jbev Exp $")
...
#	endif
#ifdef MPX
	    if (chmod(from, mode_while_renaming & 0666) != 0)
#else /* MPX */
 ...
 */
{
	...
	if (changed && !nerror) {
		...
		ignoreints();
#ifdef MPX
		/* on MPX must be 664 to have others do checkout */
