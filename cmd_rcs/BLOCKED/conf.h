...
/* Modified by J. Bevier, JB Systems, for NASA RCS port for MPX */
 
/* $Id: conf.h,v 1.9 1995/08/02 17:02:33 jbev Exp $ */
...
#define exitmain(n) exit(n)	/* how to exit from main() */
...
/* Define the following symbols to be 1 or 0.  */
#ifdef mpx
#define has_prototypes 0	/* Do function prototypes work?  */
#define has_stdarg 0		/* Does <stdarg.h> work?  */
#define has_varargs 1		/* Does <varargs.h> work?  */
#define va_start_args 1		/* How many args does va_start() take?  */
#else
...
#define text_work_stdio 0	/* Text i/o for wrk file, bin for RCS file?  */
...
#define bad_fopen_wplus 0	/* Does fopen(f,FOPEN_WPLUS) fail to trunc f? */
...
#define has_rename 0	/* Does rename() work?  */
...
#define VOID (void)	/* 'VOID e;' discards the val of an expr 'e'. */
...
#ifdef mpx
typedef char *malloc_type;	/* type returned by malloc() */
#else
...
#ifdef mpx
#define RCS_SHELL "/system/bin/sh"	/* shell to run RCS subprograms */
#else /* mpx */
...
#endif /* mpx */
#ifdef mpx
#define CO "/system/bin/co"	/* name of 'co' program */
#define DIFF "/system/bin/diff"	/* name of 'diff' program */
#define DIFF3 "/system/bin/diff3"	/* name of 'diff3' program */
#define ED "/system/bin/ed"	/* name of 'ed' prog (only if !DIFF3_BIN) */
#define MERGE "/system/bin/merge"	/* name of 'merge' program */
#else /* mpx */
...
#define ED "/bin/ed"		/* name of 'ed' prog (only if !DIFF3_BIN) */
...
#define DIFF3_BIN 1	/* Is diff3 user-visible (not the /usr/lib aux)?  */
...
#ifdef mpx
char *memcpy P((void*,void const*,size_t));
#else
