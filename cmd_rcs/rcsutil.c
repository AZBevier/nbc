...
/*
 * $Log: rcsutil.c,v $
 * Revision 1.10  2021/07/09 20:41:46  jbev
 * Fix warning errors.
 *
 * Revision 1.9  1995/10/06 23:29:51  jbev
 * Fix previous message
 *
 * Revision 1.8  1995/09/07  22:13:59  jbev
 * Left off terminator on comment, OOPS.
 *
 * Revision 1.7  1995/04/19  23:22:47  jbev
 * Correct redirections strings.
 *
 * Revision 1.6  1995/04/19  20:04:17  jbev
 * Correct CO mpx problems.
 *
 * Revision 1.5  1995/04/14  18:25:18  jbev
 * Left system call in mpx code.
 *
 * Revision 1.4  1995/04/14  05:27:15  jbev
 * Change DQE_ON to DQE_OWN for word offset.
 *
 * Revision 1.3  1995/04/14  05:22:54  jbev
 * Add code for MPX to get ownername from DQE.
 *
 * Revision 1.2  1995/04/14  05:07:49  jbev
 * More fixes for MPX
 *
 * Revision 1.1  1995/03/04  18:10:23  jbev
 * Initial revision
 *
...
libId(utilId, "$Id: rcsutil.c,v 1.10 2021/07/09 20:41:46 jbev Exp $")
	...
	int suspicious;
{
	...
	/* get users ownername in dqe.on */
	reg[1] = (int)&dqe[DQE_OWN];	/* 4wds into DQE */
	reg[2] = (int)name;		/* where name is to go */
	reg[4] = 8;			/* 8 bytes */
...
#	endif
	...
	if (inoutargs[0]) {
		bufscpy(&in, "<");
		bufscat(&in, inoutargs[0]);
		*p++ = in.string;
	}
	if (inoutargs[1]) {
		bufscpy(&out, ">");
		bufscat(&out, inoutargs[1]);
		*p++ = out.string;
	}
	*p = (char *)0;
	p = inoutargs+2;
	if ((wstatus = vexecv(b.string, p)) == -1) {
		wstatus = vexecv(RCS_SHELL, p);
	}
	if (wstatus == 0) {
		return (0);	/* all was a ok */
	}
	wstatus = (1 << 8);	/* show an error */
