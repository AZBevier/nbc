...
/*
 * $Log: rcs.c,v $
 * Revision 1.6  2021/07/09 23:46:29  jbev
 * Fix warning error.
 *
 * Revision 1.5  2021/07/09 20:41:46  jbev
 * Fix warning errors.
 *
 * Revision 1.4  1995/04/20 02:19:13  jbev
 * Fix syntax error.
 *
 * Revision 1.3  1995/04/20  01:59:17  jbev
 * Modify options for MPX.
 *
 * Revision 1.2  1995/04/18  00:07:24  jbev
 * Fix umask call.
 *
...
mainProg(rcsId, "rcs", "$Id: rcs.c,v 1.6 2021/07/09 23:46:29 jbev Exp $")
{
	static char const cmdusage[] =
#ifdef MPX
	"\nrcs usage: rcs -{ge}logins -Afile -{blu}[rev] -cstring -{iYZ} -{nNs}name[:rev] -orange -t[file] -Vn file ...";
#else
	...
	while (a = *++argv,  0<--argc && *a++=='-') {
		...
		case 'r':
			...
			if (!*a) {
#ifdef MPX
			    error("missing symbolic name after -R");
#else
			    error("missing symbolic name after -N");
#endif
                            break;
