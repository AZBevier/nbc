...
#ifndef lint
static char *rcsid =
"@(#) $Header: /u/nbc/nbccom/RCS/trees.c,v 1.9 1995/02/27 17:03:05 jbev Exp $";
#endif
...
NODE       *buildtree(o, l, r) register NODE   *l, *r;
{
    register NODE  *p, *q;
    register    actions;
    register    opty;
...
NODE       *convert(p, f) register NODE    *p;
{
...
#ifndef BUG2
#ifdef DBXDEBUG
DBprint(p)		/* for use when debugging from dbx */
	register NODE *p;
{
	fwalk(p, eprint, 0);
}
...
eprint(p, down, a, b) register NODE    *p;
int        *a, *b;
{
    register    int ty;
    char *os;
