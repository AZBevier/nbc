...
#ifndef lint
static char *rcsid = "@(#) (Gould) $Header: /u/nbc/nbccom/RCS/stab.c,v 1.1 1995/01/27 21:56:20 jbev Exp $";
#endif
...
/*		    classname = "TYPEDEF"; */
		...
		case ENAME: 
    	nn = (char *) malloc(strlen(p->sname) + 7);
    	strcpy(nn, "enum ");
    	strcat(nn, "_");
    	strcat(nn, p->sname);
...
/* printf(" outstab ENAME entertype %s\n", nn); */
    entertype(p->stype, NILINDEX, FORWARD, dimtab[p->sizoff + 3], nn);
		...
		case UNAME: 
		    ignore = true;
DOunion:
    	nn = (char *) malloc(strlen(p->sname) + 8);
    	strcpy(nn, "union ");
    	strcat(nn, "_");
    	strcat(nn, p->sname);
...
/* printf(" outstab UNAME entertype %s\n", nn); */
    entertype(p->stype, NILINDEX, FORWARD, dimtab[p->sizoff + 3], nn);
		...
		case STNAME: 
		    ignore = true;
DOstruct:
    	nn = (char *) malloc(strlen(p->sname) + 9);
    	strcpy(nn, "struct ");
    	strcat(nn, "_");
    	strcat(nn, p->sname);
...
private genstruct(t, structid, index, name, size, p)
TWORD	t;
int	structid;
int	index;
char	*name;
