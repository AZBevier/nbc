...
#ifndef lint
static char *rcsid = "$Id: scan.c,v 1.6 1995/03/06 19:18:32 jbev Exp $";
#endif
...
"Warning: Invalid Tree Node Table Size (Using Default)\n");
    ...
    stab = (struct symtab *)malloc(sizeof(struct symtab) * (SYMTSZ + 1));
    if (stab <= 0)
       cerror("Out of memory: Can not alloc Symbol Table");
    dimtab = (int *)malloc(sizeof(int) * DIMTABSZ);
    if (dimtab <= 0)
 ...
 /* match a string or character constant, up to lxmatch */
	...
	mkcc1: 
		if (lxmatch == '\'') {
		    ...
		    if(i >= STRBUFSZ) {
			werror("string initializer truncated to %d characters",STRBUFSZ);
			tbflag = 1;
			continue;
		    }
		    if (strflg) {
			if (ct == 0 || i < ct)
			    ...
			    if (i == ct)
			werror("string initializer truncated to %d characters",ct);
		    }
		    else
			newstr[i] = val;
		}
...
yylex()
{
	...
	/* eat up a single character, and return an opcode */
	    ...
	    case A_ERR: 
		uerror("illegal character: %02x (hex)", lxchar);
		break;
	...
	/* = */
		...
		werror("Old fashioned assignment operator =%c", id);
		return(ASOP);
 
	    default: 
		cerror("yylex error, character %02x (hex)", lxchar);
