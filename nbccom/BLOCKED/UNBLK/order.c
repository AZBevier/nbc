...
#ifndef lint
static char *rcsid = "@(#) (Gould) $Header: /u/nbc/nbccom/RCS/order.c,v 1.2 1995/01/27 23:17:40 jbev Exp $";
#endif
 ...
 */
delincr(p, cook) register NODE   *p;
{
	register NODE *pl;
	register int op, lop;
	NODE	*ptemp;
...
#ifndef MAR28
#ifdef SPR02_28_94_NEW
	if ((lo == UNARY MUL && ro == UNARY MUL) &&
    	    p->in.left->in.su <= p->in.right->in.su) {
	    offstar(p->in.right->in.left);
	    return(1);
    	}
    	if (lo == UNARY MUL && ro == UNARY MUL) {
    	    p->in.left->tn.rall = R2 | MUSTDO;
