...
int isnzs( s, n ) reg char *s; int n; {
	reg char	*se;
...
static int
addsk( s, n, k ) reg char *s; int n; reg uns k; {
	reg char	*se;
...
static int
mulsk( s, n, radix, k ) reg char *s; int n, radix; reg uns k; {
...
int divsk( s, n, radix, k ) reg char *s; int n, radix; reg uns k; {
...
int dodig( f, k, tz ) reg FPN *f; int k, tz; {
	if( f->ne ){
		f->ne++;
		return 0;
	}
...
static int
mvb( tgt, src, n ) register char *tgt, *src; int n; {
	register char *bnd;
...
static int
cmpss( s1, n1, s2, n2 ) reg char *s1, *s2; int n1, n2; {
	reg char	*se1, *se2;
	reg		int i;
...
static int
subss( s1, n1, s2, n2, k ) reg char *s1, *s2; reg /*uns*/ int k; int n1, n2; {
	reg char	*se1, *se2;
...
static int
addss( s1, n1, s2, n2, k ) reg char *s1, *s2; reg /*uns*/ int k; int n1, n2; {
	reg char	*se1,
			*se2;
