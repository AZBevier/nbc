/*~!ln.c*/
/* Name:  ln.c Part No.: _______-____r
 *
...
#ident	"Make4MPX $Id: ln.c,v 1.3 2021/07/04 19:39:21 jbev Exp $"

/* $Log: ln.c,v $
 * Revision 1.3  2021/07/04 19:39:21  jbev
 * Remove warnings.
 *
 * Revision 1.2  1995/10/17 23:23:24  jbev
...
char *argv[];
{
    ...
    if((ip = cvtfname(argv[optind])) == (char *)0) {
    	fprintf(stderr, "ln: can't resolve path %s\n", argv[optind]);
    	usage();
    }
    ...
    if ((argc-optind) > 2) {
    	...
    	if (stat(argv[argc - 1], &s2) < 0) {
    	    fprintf(stderr, "ln: directory %s not found\n", argv[argc - 1]);
    	    exit(2);
    	} 
...
char *source, *target;
{
    ...
    if (udflag) {
    	...
    	if ((from = open(source, iflag, 0666)) < 0) {
    	    fprintf(stderr, "ln: cannot open %s\n", source);
    	    return 1;
    	}
    } else {
    ...
    if (mflg)
#ifdef mpx
    	chmod(target, s1.st_mode & 0777);	/* just mode */
#else
...
void usage()
{
    fprintf(stderr, "Usage: ln [-aou] f1 f2\n       ln [-aou] f1 ... fn d1\n");
    exit(2);
}
