/*~!chgrp.c*/
/* Name:  chgrp.c Part No.: _______-____r
 *
...
#ident	"Make4MPX $Id: chgrp.c,v 1.2 2021/07/02 18:44:33 jbev Exp $"

/*
 * $Log: chgrp.c,v $
 * Revision 1.2  2021/07/02 18:44:33  jbev
...
/*
 * chgrp [-d] project file/[directory] ...
 */
...
char *argv[];
{
    ...
    if(argc < 3) {
    	fprintf(stderr,"usage: chgrp [-d] project file/[directory] ...\n");
    	exit(2);
    }
...
#ifndef mpx
    if(isnumber(pp)) {
    	gid = atoushort(pp); /* gid is unsigned short */
    } else {
    	if((gr=getgrnam(pp)) == NULL) {
...
#ifdef mpx
    	if(chownm(fp, (char *)0, pp) < 0) {
#else
    	if(chown(fp, stbuf.st_uid, gid) < 0) {
#endif
    	    perror(argv[c]);
    	    status = 1;
    	}
