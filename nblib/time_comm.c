/*~!time_comm.c*/
/* Name:  time_comm.c Part No.: _______-____r
 *
...
char 		*tzname[] = {"GMT","   "};
static short	month_size[12] = { 31, 28, 31, 30, 31, 30,
    				   31, 31, 30, 31, 30, 31 };
...
time_t	*tim;
{
	...
	curr = ct->tm_yday*SEC_IN_DAY + ct->tm_hour*60*60
...
tzset()
{
  ...
  char *ptr;
	...
	if ( ((ptr = getzname(ptr,tzname[0])) == 0) ||
    	    ((ptr = gettime(ptr,&timezone,1)) == 0))
		return;
...
struct tm *t;
{
    ...
    while (t->tm_year < daytab[i].yrbgn) /* can't be less than 7 */
		i++;
    /* fall through the loop when in correct interval	*/
    *s = daytab[i].daylb;
    *e = daytab[i].dayle;
