/*~!doprnt.c*/
/* Name:  doprnt.c Part No.: _______-____r
 *
...
#define PUT(p, n) {unsigned char *nbp;if((nbp=bufptr+n)>bufferend){\
_dowrite(p,n,iop,&bufptr);}else{\
(void)memcpy((char *)bufptr,p,n);bufptr=nbp;}}

#define PAD(s,n) {int x;for(x=n;x>20;x-=20)\
_dowrite(s,20,iop,&bufptr);PUT(s, x);}
