/*~!rmdir.c*/
/* Name:  rmdir.c Part No.: _______-____r
 *
...
#ident	"Make4MPX $Id: rmdir.c,v 1.2 2021/07/05 20:31:05 jbev Exp $"

/*
 * $Log: rmdir.c,v $
 * Revision 1.2  2021/07/05 20:31:05  jbev
 * Fix warning errors.
 *
...
char **argv;
{
    ...
    while (--argc) {
    	/* convert given name to a directory path */
    	argv++;
    	/* convert arg to a directory */
    	if((ip = cvtdname(*argv)) == (char *)0) {
    	    fprintf(stderr, "rmdir: can't resolve path %s\n", *argv);
    	    exit(2);
    	}
    	/* see if arg is a directory */
    	if (stat(ip, &s1) < 0) {
    	    fprintf(stderr, "rmdir: directory %s not found\n", *argv);
/*    	    fprintf(stderr, "rmdir: directory %s not found\n", ip); */
    	    exit(2);
    	} 
    	...
    	if (rmdir(ip) < 0) {
    	    fprintf(stderr, "rmdir: can't remove directory %s", *argv);
    	    errors++;
    	}
    	free(ip);
    }
